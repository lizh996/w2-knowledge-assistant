...
flowchart TB
    ...
    subgraph s2["管线层 - 离线数据管线"]
        P1[解析 fitz] --> P2[分块 结构分块+512token]
        P2 --> P3[向量化 BGE-M3]
        P3 --> P4[入库 Qdrant]
    end
    subgraph s3["服务层 - 在线服务"]
        API[FastAPI /ask]
        RET[检索 dense 相似度]
        GEN[生成 DeepSeek 带页码]
    end
...
**V1 说明**：朴素检索（提问→答案→页码溯源），先跑通、先建基线。
...
flowchart TB
    ...
    subgraph s3["服务层"]
        API[FastAPI]
        RET[复合检索 dense+sparse RRF]
        TRACE[trace 可观测 8步+四路]
        GEN[生成]
    end
    subgraph s4["前端"]
        UI[问答页+证据折叠]
...
**V2 新增**：trace 可观测（为迭代归因）+ 复杂元素提取 + 复合检索（dense+sparse RRF）+ 运维控制台。
